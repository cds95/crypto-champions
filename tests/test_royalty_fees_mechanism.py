...
def test_royalty_paid_out(accounts, crypto_champions, mint_first_elder):
    ...
    royaltyFee = mintPrice * 0.8 # Hardcoded here should be equal to the constant in the contract
    assert accounts[0].balance() == balBeforeHeroMint + royaltyFee


def test_elder_ownership_transfer_royalty(accounts, crypto_champions, mint_first_elder):
    ...
    mintPrice = crypto_champions.getHeroMintPrice(crypto_champions.currentRound(), crypto_champions.eldersInGame())
    crypto_champions.mintHero(crypto_champions.eldersInGame(), {"from": accounts[2], "value": mintPrice})
    royaltyFee = mintPrice * 0.8 # Hardcoded here should be equal to the constant in the contract
